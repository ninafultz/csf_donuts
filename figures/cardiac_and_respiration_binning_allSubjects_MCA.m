...
subjectDirs = subjectDirs(contains({subjectDirs.name}, '20191029_Rec'));
  ...
  % try
            ...
            if strcmp(subject_code, '20191029_Rec')
                roi_pvsas_file = find_roi_file(fullfile(projPath, subject_code, 'ROIs_neworientation', ...
                    roi_internal{r}));
                roi_sas_file   = find_roi_file(fullfile(projPath, subject_code, 'ROIs_neworientation', ...
                    roi_external{r}));
            else
                roi_pvsas_file = find_roi_file(fullfile(projPath, subject_code, 'ROIs', ...
                    roi_internal{r}));
                roi_sas_file   = find_roi_file(fullfile(projPath, subject_code, 'ROIs', ...
                    roi_external{r}));
            end
            ...
            plottingADCandFAacrossPhasesAllSubjects(physio, signal, physio_bin, ...
                roi_pvsas, roi_sas, nPhases, phasesLoaded, roiName, subject_code);
    %  catch ME
