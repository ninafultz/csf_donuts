%%% plotting cardiac, respiration, and random over phases for PVSAS vs. SAS
%% outline:
    % 1) load 6 phases for cardiac e.g. R:\- Gorter\- Personal folders\Fultz,
    % N\csfdonuts_lydiane\20201014_Reconstruction\Cardiac\T2prep\Results\DTIresult_phase1.mat
    % DTIresult_phase2.mat
    % 2) load 6 phases for respiration: R:\- Gorter\- Personal folders\Fultz,
    % N\csfdonuts_lydiane\20201014_Reconstruction\Respiration\T2prep\Results\DTIresult_phase1.mat
    % DTIresult_phase2.mat
    % 3) load 6 phases for the random
    % 4) apply PVSAS and SAS ROI ('called m2_to both the ADC and FA over phases (need
    % to triple check that the ROI is in the correct place and
    % orientation): R:\- Gorter\- Personal folders\Fultz,
    % N\csfdonuts_lydiane\20201014_Reconstruction\ROIs\m2_left_internadonut.nii
    % 5) make a plot with subplots showing all voxels for PVSAS and the
    % mean over cardiac, respiration, and random with mean over phases for
    % ROI 
    % 6) make plot but now have the SAS be normalized by the mean across
    % phases (cardiac, respiration, random)
    % 7) make a plot with subplots showing all voxels for PVSAS and the
    % mean over cardiac, respiration, and random with mean over phases for
    % ROI 
    % 8) make plot but now have the SAS be normalized by the mean across
    % phases (cardiac, respiration, random)

clear
subjects(:,1) = {'20201014'}; %1st column=cardiac/respi

GENERALDIR = 'R:\- Gorter\- Personal folders\Fultz, N\scripts\csfdonuts_lydiane\';
tmpSubCardiac = dir([GENERALDIR '*20201014_Reconstruction']);
...
%%plotting: plotting cardiac, respiration, and random over cardiac phases 