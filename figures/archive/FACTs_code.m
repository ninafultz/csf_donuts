...
cd('R:\- Gorter\- Personal folders\Fultz, N\csfdonuts_lydiane\20201014_Reconstruction\Cardiac\T2prep\Results\'); % The path of TENSOR.mat file

% Initialize cell array with filenames
tensorFiles = { ...
    'TensorPhase1NoNeg.mat', ...
    'TensorPhase2NoNeg.mat', ...
    'TensorPhase3NoNeg.mat', ...
    'TensorPhase4NoNeg.mat', ...
    'TensorPhase5NoNeg.mat', ...
    'TensorPhase6NoNeg.mat'};

% Preload first file to get dimensions
load(tensorFiles{1});  % assume it loads variable named e.g., Tensor
[X,Y,Z] = size(tensor_registered); 

DTIvol = zeros(X,Y,Z,length(tensorFiles),'single');
[Xdim,Ydim,Zdim,~] = size(DTIvol);
FE = zeros(Xdim,Ydim,Zdim,3,'single');  % first eigenvector
VDims = [0.45 0.45 0.45];  % mm
MDims = [450 556 422]; % size of your data 

load("FA_cardiacThres50.mat");
FA = FA_cardiac;

tic;
Th_FLmin = 5;% Threshold of minimum fiber length (mm)
...
SeedY = 30:100;
SeedZ = 2:54;
fnam = 'Fiber_1dir_WholeBrain'; % Save mat-file
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% ----------
numSeeds = size(Seed,1); 

Tracts    = cell(numSeeds,1);  % each cell will hold Nx3 positions
TractL    = zeros(numSeeds,1); % length in steps
TractAng  = cell(numSeeds,1);  % angle between steps
TractFA   = cell(numSeeds,1);  % FA along fiber
TractFE   = cell(numSeeds,1);  % FEFA along fiber
InMa      = cell(size(FA));    % initialize as empty, will fill with fiber indices
CMa       = zeros(size(FA),'single'); % counts of fibers passing each voxel
        ...
        if dir == 2;
         ...
         while (i <= MDims(1) && i >= 1 && j <= MDims(2) && j >= 1 && k >= 1 && k <= MDims(3));   
             Int_i = ceil(i);Int_j = ceil(j);Int_k = ceil(k);% ceil the number of current point
                   if dir == 2;
                      Fsre = sre + 1;% Increment of single fiber segment in the second direction
                   end
