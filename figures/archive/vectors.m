...
%% MCA
slicesCor = 300:325; % 26
slicesTra = 290:325; % 36
slicesSag = 160:180; % 21
suffixeVector= '_test_V3';
...
for p = 1:6
    ...
    load([subject.folder '\' subject.name  physioFolder '\Results\' sprintf('DTIresult_phase%d%s.mat',...
        p, suffixeTensor)]); % will load VectorF etc

    % [x_dim,y_dim,z_dim] = size(referenceScan);
    % mask = referenceScan > threshold;        % size 72 × 52 × 42
    % 
    % mask_resized = imresize3(mask, [x_dim, y_dim, z_dim/2]);  % resize 3rd dimension to 21
    % mask_resized = repmat(mask_resized, [1, 1, 1, 3]);  % expand 4th dimension to match VectorF_cropped
    VectorF_thresh = VectorF(slicesTra,slicesCor, slicesSag, :); %.* mask_resized;


    VectorF_thresh(VectorF_thresh==0) = NaN;

    clear VectorF_cropped


    RGBmap_thresh = nan(size(VectorF_thresh,1),size(VectorF_thresh,2),3,size(VectorF_thresh,3));  
    for i = 1:size(VectorF_thresh,1)
        for j = 1:size(VectorF_thresh,2)
            for k = 1:size(VectorF_thresh,3)
...
mkdir([newFolder '\' subject.name '\vectorsRGB']);
save([newFolder '\' subject.name '\vectorsRGB\Vectors_RBGThresh150' suffixeVector '.mat'], ...
