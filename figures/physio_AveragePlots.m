...
for s = 1:length(signals)
    ...
    for p = 1:length(physioConditions)
        ...
        for r = 1:length(roiPairs)
            ...
            idxExternal = allAligned.roi == externalROI & allAligned.physio == physioName & allAligned.signal == sigName;
            
            if ~any(idxInternal) || ~any(idxExternal)
                fprintf('No data for %s or %s', internalROI, externalROI);
            end
                      
            tsInternalCell = allAligned.ts(idxInternal);
            tsInternal = vertcat(tsInternalCell{:});  % size: totalCycles x 6

            tsExternalCell = allAligned.ts(idxExternal);
            tsExternal = vertcat(tsExternalCell{:});
            
            meanInternal = mean(tsInternal,1);
            meanExternal = mean(tsExternal,1);
    ...
    for p = 1:length(physioConditions)
        ...
        for r = 1:length(roiPairs)
            ...
            for s = 1:length(signalsToPlot)
                ...
                if ~any(idxInternal) || ~any(idxExternal)
                    warning('No data for %s or %s', internalROI, externalROI);
                    continue;
                end
                
                tsInternal = vertcat(allAligned.ts{idxInternal});
                tsExternal = vertcat(allAligned.ts{idxExternal});
                
                meanInternal = mean(tsInternal,1); meanInternal = meanInternal(:)';
                meanExternal = mean(tsExternal,1); meanExternal = meanExternal(:)';
                ...
                p2 = plot(phases, meanExternal, '-o', 'MarkerFaceColor', [1 0.5 0], 'MarkerEdgeColor', [1 0.5 0], 'MarkerSize', 5);

                % Plot individual trials (semi-transparent lines)
                plot(phases, tsInternal', '-', 'Color', [0 0 1 0.2]);
                plot(phases, tsExternal', '-', 'Color', [1 0.5 0 0.2]);
                legend([p1, p2], {'PVSAS', 'SAS'}, 'Location','best');
            end
...
for g = 1:2
    ...
    for r = 1:length(roiPairs)
        ...
        title(sprintf('%s vs %s', internalROI, externalROI));
            ...
            for p = 1:length(physioConditions)
                ...
                for s = 1:length(signalsToPlot)
                    ...
                    idxExternal = allAligned.roi == externalROI & ...
                                  allAligned.physio == physioName & ...
                                  allAligned.signal == sigName;
                    ...
                    nExternal = size(tsExternal,1);
            
                    ciInternal = 1.96 * std(tsInternal) / sqrt(nInternal);
                    ciExternal = 1.96 * std(tsExternal) / sqrt(nExternal);
                    ...
                    fill([phases fliplr(phases)], [meanExternal+ciExternal fliplr(meanExternal-ciExternal)], ...
                        colors(p,:), 'FaceAlpha', 0.2, 'EdgeColor','none');
            
                    % Plot internal/external means
                    plot(phases, meanInternal, '-o', 'Color', colors(p,:), 'MarkerFaceColor', colors(p,:));
                    plot(phases, meanExternal, '-s', 'Color', colors(p,:), 'MarkerFaceColor', colors(p,:));
                    plot(phases, tsInternal, '-', 'Color', [colors(p,:) 0.3]);
                    plot(phases, tsExternal, '-', 'Color', [colors(p,:) 0.3]);
        ...
        for p = 1:length(physioConditions)
            ...
            legendEntries{end+1} = sprintf('%s – external', physioConditions{p});
        end
        ...
        title(sprintf('%s vs %s', internalROI, externalROI));
            ...
            for p = 1:length(physioConditions)
               ...
               % Example ROI pattern: m1_left_internaldonut or m2_right_internaldonut
                ...
                tokens = regexp(roiPairs{r}{1}, '^(m\d+)_', 'tokens', 'once');
                baseROI = tokens{1};
