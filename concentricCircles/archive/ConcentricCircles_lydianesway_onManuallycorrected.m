...
ROI = ROIs{r};
   ...
   % Default filenames
            ...
            switch subject_code
                case {'20191112_Reconstruction', '20191029_Rec', '20201008_Reconstruction', '20201016_Reconstruction', '20201014_Reconstruction'}
                    t1File = 't1.nii'; % rT1
...
vessel_file = fullfile(CCDir, ['vesseltoCheck_' ROI '_manuallyedited.nii']);
 if exist(vessel_file, 'file')
            ...
            vessel_file = fullfile(CCDir, ['vesseltoCheck_' ROI '_manual_mask.nii']);
            V = spm_vol(vessel_file);
            vesseltoKeep = spm_read_vols(V);

            t1mask_inROI = vessel .* (adc == 0);
            % vesselKept = t1mask_inROI .* (vesseltoKeep == 1);
            % 
            cd(CCDir);
             volumeToSave = single(t1mask_inROI);
             niftiwrite(volumeToSave, ['vesselKept_' ROI '.nii']);
...
%%   
                           ...
                           % CSFmask(isnan(CSFmask)) = 0;
                            ...
                            diameter = num2str(d);
                            niftiwrite(CSFmask, ['CSFMask_' diameter '_' ROI '_manuallycorrected.nii']);
