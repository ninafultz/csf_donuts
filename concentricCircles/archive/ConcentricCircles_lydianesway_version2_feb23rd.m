...
ROI = ROIs{r};
       ...
       % t1mask_inROI = t1vesselmask .* (adc == 0);
...
combined_filled = imfill(combined_closed, 'holes');
        ...
        % imshow3Dfull(filledVessel); 
                ...
                cd(CCDir);
                niftiwrite(boundarySingle, ['vesseltoCheck_' ROI '.nii']);
...
%%   
                           ...
                           % CSFmask(isnan(CSFmask)) = 0;
                            ...
                            diameter = num2str(d);
                            niftiwrite(CSFmask, ['CSFMask_' diameter '_' ROI '.nii']);
                % % 
        ...
        % plotting 
         ...
         figure; hold on;
                    ...
                    yyaxis left
                    plot(binCenters, averageADC_MCA);
                    ylabel('csf-mobility');
                    ylim([0.02 0.06]);
                    xlim([0.45 4.5]);
                    ...
                    xlabel('Dilation (mm)');
                    title(['CSF ' ROI ' subject:' subject_code])
                    hold off;
...
save(['allCSFmobilityLydiane' ROI '_Median_combined.mat'], 'averageADC_MCA');
save(['allB0Lydiane' ROI '_Median_combined.mat'], 'averageCSF_MCA');
%% plotting all subjects
 ...
 meanADC = mean(averageADC_MCA);
            ...
            xlabel('Dilation (mm)');
            title(['CSF - Lydianes way - All Subjects ' ROI])
            hold off;
