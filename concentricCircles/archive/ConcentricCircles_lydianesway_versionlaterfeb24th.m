...
ROI = ROIs{r};
        ...
        %%
                ...
                for iter = 1:5
                    ...
                    % Count adc==0 voxels in local neighborhood of each candidate voxel
                    localADCzeroCount = imdilate(adcZeroMask, strel('sphere', 1));  
                    
                    % Better - actual count using convolution
                    localCount = convn(single(adcZeroMask), ones(3,3,3), 'same');
                    
                    % Only add new voxels that: aren't already in mask, not in adc>0, 
                    % and don't have more than 3 adc==0 neighbors
                    newVoxels = expanded & ~dilatedMask & ~adcMask & (localCount <= 1);
                ...
                % % Find vessel boundary voxels that have adc>0 nearby
                % vesselBoundary = imdilate(logical(dilatedMask), strel('sphere',1)) & ~logical(dilatedMask);
                % validStart = vesselBoundary & adcMask;
                % 
                % % Use validStart as your seed instead of vessel
                % prevDilated = logical(vessel) | validStart;
                
                boundarySingle = single(dilatedMask);
                cd(CCDir);
                niftiwrite(boundarySingle, 'vessel_ROI_feb25th.nii');
...
%%   
                           ...
                           % CSFmask(isnan(CSFmask)) = 0;
                            ...
                            diameter = num2str(d);
                            niftiwrite(CSFmask, ['CSF_mask_' diameter '_round2.nii']);
                % % 
...
save(['allCSFmobilityLydiane' ROI '_Median.mat'], 'averageADC_MCA');
save(['allB0Lydiane' ROI '_Median.mat'], 'averageCSF_MCA');
