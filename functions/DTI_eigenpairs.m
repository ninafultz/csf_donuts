function [VectorF1,VectorF2,VectorF3]=DTI_eigenpairs(DTIdata,parameters)
% This function will perform DTI calculations on a certain
% DTI dataset.
% it is going to extract lambda 1, 2, and 3 
%
        ...
        for z=1:size(S0,3)
            ...
            if(S0(x,y,z)>parameters.BackgroundTreshold)
                ...
                if(FAv>parameters.WhiteMatterExtractionThreshold)
                    FA(x,y,z)=FAv;
                    VectorF1(x,y,z,:)=EigenVectors(:,3)*EigenValues_old(3); % lambda 1
                    VectorF2(x,y,z,:)=EigenVectors(:,2)*EigenValues_old(2); % lambda 2
                    VectorF3(x,y,z,:)=EigenVectors(:,1)*EigenValues_old(1); % lambda 3
                end
            end
        end
    end
