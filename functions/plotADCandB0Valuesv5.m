function plotADCandB0Valuesv5(project_directory, project_name, subject_list, roi_patterns, voxel_size)
%% 
   ...
   % b0 = niftiread(fullfile(reoriented_dir, 'B0_properOrientation_thr75.0000.nii'));
    ...
    for roi_name = roi_patterns
        ...
        for i = 1:length(roi_files)
            ...
            % find plane with stuff in it
            % read A = readmatrix(filename)
            % Assume ROI is a 3D binary mask (logical or uint16)
            % individually define subject slice which is hokey but fine
%nonEmptySlices = find(squeeze(any(any(ROI, 1), 2)));
            nonZeroVoxels = find(ROI(165,:,:)); 
