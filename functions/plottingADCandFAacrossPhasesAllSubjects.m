function plottingADCandFAacrossPhasesAllSubjects(physio, signal, physio_bin, roi_pvsas, ...
    roi_sas, nPhases, phasesLoaded, roiName, subject_code);
...
% Preallocate voxel matrix
nVoxels = nnz(roi_sas);      % number of voxels in ROI
voxel_values_sas = zeros(nVoxels, nPhases);

% Extract voxel values for each phase
for t = 1:nPhases
    vol = phasesLoaded{1,t}.data;
    voxel_values_sas(:,t) = vol(roi_sas);
end

% Normalize across voxels (optional)
voxel_values_norm_sas = voxel_values_sas ./ mean(voxel_values_sas,2);

% Mean across voxels per phase
card_sas = mean(voxel_values_sas,1,'omitnan');
norm_sas_cardiac = mean(voxel_values_norm_sas,1,'omitnan');
...
subplot(1,4,2)
plot(1:nPhases, voxel_values_sas', 'Color', [0.6 0.6 1]); hold on
plot(1:nPhases, card_sas, 'b-', 'LineWidth', 2)
ylim([ymin ymax])
subtitle(sprintf('SAS: %s ', signal));

% Normalized SAS voxel-wise
subplot(1,4,4)
plot(1:nPhases, voxel_values_norm_sas', 'Color', [0.6 0.6 1]); hold on
plot(1:nPhases, norm_sas_cardiac, 'b-', 'LineWidth', 2)
ylim([0.8 1.2])
subtitle(sprintf('Normalized SAS: %s ', signal));
