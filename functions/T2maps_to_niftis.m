function T2maps_to_niftis(subjPath);
...
%% Paths
t2maps_path    = fullfile(subjPath, 't2_maps');
...
% Target file path
echo1File = fullfile(regDir, 'pp_*_b0_normalV4.nii');
...
fileExists = ~isempty(dir(echo1File));  % Use 'dir' to handle wildcards
    ...
    % Find the source file in the anat folder
        sourceFiles = dir(fullfile(anatFolder, 'pp_*_b0_normalV4.nii'));
...
for i = 1:length(mat_files)
    ...
    niftiInfo.ImageSize = size(ADC_map);
    niftiInfo.Datatype = class(ADC_map);  % Match the datatype to the T2 map
    ...
    niftiwrite(ADC_map, output_nii_file, niftiInfo);
...
for i = 1:length(mat_files)
    ...
    % Get the field name dynamically (since the variable name may vary)
    var_name = fieldnames(loaded_data);  % Get the name of the variable (e.g., 'T2_map_CSF')
    FA_map = loaded_data.FA;  % Access the FA map data
