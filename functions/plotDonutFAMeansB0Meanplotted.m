function plotDonutFAMeansB0Meanplotted(project_directory, project_name, subject_list, roi_patterns, voxel_size);
roi_data = struct();
...
for idx = 1:num_rois
    ...
    figure('Name', ['FA Pattern: ', roi_name], 'NumberTitle', 'off');
    hold on;
...
title('Comparison of FA and B0 Values');
legend('Location', 'best');
    ...
    % Plot the mean ADC values with a solid line
    yyaxis left
    plot(unique_x_mm, mean_adc, '-o', 'LineWidth', 1, 'DisplayName', 'Mean FA', 'Color', 'r');

    % Add labels and title
    xlabel('Distance from Zero (mm)');
    ylabel('FA Intensity');
    title(['FA Cross-Sectional Pattern: ', roi_name]);
    ...
    fill([unique_x_mm, fliplr(unique_x_mm)], ...
         ...
         [0.5, 0.8, 1], 'FaceAlpha', 0.3, 'EdgeColor', 'none', 'DisplayName', 'B0 average');
    hold on;
    ylabel('B0');
