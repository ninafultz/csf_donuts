...
# Find all echo files (search for .nii instead of .nii.gz)
echo_files=$(find . -maxdepth 1 -type f -name '*_e*.nii')
...
# Extract unique prefixes based on the common part before _e<number>.nii
prefixes=($(echo "$echo_files" | sed 's|^\./||' | sed 's/_e[0-9a-zA-Z]*\.nii//g' | sort -u))

# Print the prefixes to verify
echo "${prefixes[@]}"
...
for prefix in "${prefixes[@]}"; do
    ...
    if [ ! -f "$merged_name" ]; then
        echo "Error: Failed to create merged file '$merged_name'"
        exit 1
    fi

    echo "Echoes merged successfully for prefix '$prefix' to '$merged_name'"

    mri_convert "$merged_name" "${merged_name%.gz}"
done
