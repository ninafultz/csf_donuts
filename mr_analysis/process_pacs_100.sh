...
for SUBJECT_DIR in "$BASE_DIR"/*/; do
    ...
    FILES_TO_COPY=(
        "B0_from_mhd.nii"
        "masked_b0_ADC_mhd_thr100.0000.nii"
        "$T1_FILE"
    )
    ...
    if [[ -f "$PACS_DIR/masked_b0_ADC_mhd_thr100.0000.nii" ]]; then
        fslmaths "$PACS_DIR/masked_b0_ADC_mhd_thr100.0000.nii" -mul 10000 "$PACS_DIR/masked_b0_ADC_mhd_thr100.0000_multi10000.nii.gz"

        # Convert the compressed .nii.gz to uncompressed .nii using mri_convert
        mri_convert "$PACS_DIR/masked_b0_ADC_mhd_thr100.0000_multi10000.nii.gz" "$PACS_DIR/masked_b0_ADC_mhd_thr100.0000_multi10000.nii"

        # Optionally, remove the .nii.gz version after conversion
        rm "$PACS_DIR/masked_b0_ADC_mhd_thr100.0000_multi10000.nii.gz"
    else
        echo "Warning: masked_b0_ADC_mhd_thr100.0000.nii not found in $PACS_DIR"
    fi
