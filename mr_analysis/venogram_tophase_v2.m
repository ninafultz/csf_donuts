...
plot_flag         = 0; % Set to 1 to enable plotting, 0 to disable
...
[data_in, info] = loadParRec(char([par_path,'/', input_filename]));% loads the raw data of all echoes (NOT YET MASKED!)
...
if plot_flag == 1
    % Flip the volume along the y-axis (left-right)
    flippedVolume = flip(averagedVolume, 1);  % Flips the 2nd dimension (y-axis)
    % Display the flipped volume
    figure;
    imshow3Dfull(flippedVolume);  % Adjust the intensity range as needed
end
